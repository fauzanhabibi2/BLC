...
function setupFloatersPhysics() {
  ...
  const container = document.querySelector('.floater-bg') || document.querySelector('.floaters');
  ...
  let mouse = { x: -1e6, y: -1e6 }; // jauh di luar layar
  ...
  let scrollV = 0;
  // Interaksi kursor: beri gaya tolak/seret lembut saat kursor mendekat
  ...
  function step(now) {
    ...
    for (const p of state) {
      ...
      // Gaya dari kursor (repulsi lembut)
      if (mouseActive) {
        const dx = p.x - mouse.x;
        const dy = p.y - mouse.y;
        const d2 = dx*dx + dy*dy;
        const rad = 180; // radius pengaruh
        const r2 = rad * rad;
        if (d2 < r2) {
          const d = Math.max(18, Math.sqrt(d2));
          const strength = (1 - d / rad) * 0.18; // skala gaya
          p.vx += (dx / d) * strength;
          p.vy += (dy / d) * strength;
        }
      }
...
function setupFormSubmit() {
  ...
  if (!form) return;
  const statusEl = form.querySelector('.form-status') || document.createElement('div');
  statusEl.classList.add('form-status');
  if (!statusEl.parentElement) form.appendChild(statusEl);
  ...
  const waLink = form.getAttribute('data-wa') || '';
  if (waBtn) {
    if (waLink) { waBtn.href = waLink; waBtn.style.display = 'inline-flex'; }
    else { waBtn.style.display = 'none'; }
  }
  if (closeBtn && success) {
    closeBtn.addEventListener('click', () => { success.hidden = true; });
    success.addEventListener('click', (e) => { if (e.target === success) success.hidden = true; });
  }
  form.addEventListener('submit', async (e) => {
    e.preventDefault();
    const url = (form.getAttribute('data-sheet-url') || '').trim();
    if (!url) {
      statusEl.textContent = 'Endpoint Google Apps Script belum diset.';
      statusEl.classList.remove('ok');
      statusEl.classList.add('err');
      return;
    }
    // Kirim sebagai application/x-www-form-urlencoded agar dibaca e.parameter di Apps Script
    const params = new URLSearchParams();
    const fields = new FormData(form);
    fields.forEach((v, k) => params.append(k, String(v)));
    params.append('timestamp', new Date().toISOString());
    params.append('source', 'daftar');
    ...
    try {
      await fetch(url, {
        method: 'POST',
        body: params,
        mode: 'no-cors' // biarkan "simple request" tanpa preflight
      });
      form.reset();
      statusEl.textContent = 'Terima kasih! Data berhasil dikirim.';
...
function playSuccessTone() {
  ...
  // C5 + E5 chord, slight glide up for a pleasant “success” feel
  o1.frequency.setValueAtTime(523.25, now);
  o2.frequency.setValueAtTime(659.25, now);
  o1.frequency.exponentialRampToValueAtTime(783.99, now + 0.5);
  o2.frequency.exponentialRampToValueAtTime(987.77, now + 0.5);
